/********************************************
Program growth_pattern2_solid_nodules.sas
Date: 06.19.2019
Author: L Ngo
Purpose: Analysis of growth pattern data for Alex Bankier -- looking specifically at rate of growth for solid nodules
...
ods rtf file="H:\Constance DeMargerie\Solid Nodules Analysis.rtf" toc_data;
...
proc mixed data=ab.solid_tumors1;
   class id;
   model log_volume = TIME_DAYS / solution;
   random int / subject=id;
    where id not in (4,34,13,31,55);  *take out nodules from IDs with more than 1 nodule; 
   title 'Model 1: Exponential Model with random intercept'; 
run;


proc corr data=ab.solid_tumors1;
   var TIME_DAYS TIME_DAYS2;
RUN;
...
data a1;
   ...
   */
   ...
   label histology_group ='HISTOLOGY 1:In Situ 2:Min Inv 3: Invasive'; 
...
data b1;
   set ab.all_tumors1 (keep=id time_days OVERALL_VOLUME_ON_FIRST_CT);
   if time_days=0;
run;
...
data ab.solid_tumors1;
   set solid_tumors1; 
   time_days2=time_days**2; 
   log_volume = log(SOLID_COMPONENT_VOLUME_ON_CT); 
   label log_volume = 'log(SOLID_COMPONENT_VOLUME_ON_CT)'; 
...
proc contents data=ab.solid_tumors1 position;
run;
...
vol_firstct             
age70_group             
solid_component_first   
visit                   
time_days2              
log_volume              
*/

proc freq data=ab.solid_tumors1;
 tables
...
proc means data=ab.solid_tumors1 n mean std median p25 p75 min max maxdec=2 ;
...
proc means data=ab.solid_tumors1 maxdec=2 n mean std median p25 p50 p75;
   var log_volume;
run;

proc univariate normal plot data=ab.solid_tumors1;
   var log_volume;
run;
...
axis1 value=(h=1.5) label=(h=1.5 a=90 "SOLID TUMOR VOLUME (mm3)") minor=none;
axis2 value=(h=1.5) label=(h=1.5 a=0 "TIME (days)") minor=none;
proc gplot data=ab.solid_tumors1;
  plot (SOLID_COMPONENT_VOLUME_ON_CT) * TIME_DAYS = ID  / vaxis=axis1 haxis=axis2 nolegend;
  *where &mindays <= OVERsolid_VOLUME_ON_CT <= &maxdays; 
  title1 h=1.5 "Growth Curves";
run;


axis1 value=(h=1.5) label=(h=1.5 a=90 "Log SOLID TUMOR VOLUME (mm3)") minor=none;
axis2 value=(h=1.5) label=(h=1.5 a=0 "TIME (days)") minor=none;
proc gplot data=ab.solid_tumors1;
...
proc freq data=ab.solid_tumors1;
   tables histology_group age_group age70_group number_ct solid_component_first;
   where time_days=0; 
...
*%expobyc(NB_OF_CTS);
*%expobyc(OVERsolid_FOLLOWUP_TIME);
...
*%expobyc(OVERsolid_VOLUME_ON_FIRST_CT);
*%expobyc(OVERsolid_VOLUME_ON_LAST_CT);
...
*%expoby3(HISTOLOGY_GROUP,In_Situ,Min_Invasive,Invasive); 

%expoby(HISTOLOGY_GROUP,Min_Invasive,Invasive); 
...
PROC EXPORT data=ab.all_tumors1

            OUTFILE= "h:\Constance DeMargerie\all_tumors.xlsx"
...
PROC EXPORT data=ab.solid_tumors1

            OUTFILE= "h:\Constance DeMargerie\solid_tumors.xlsx"
